...
\subsection{Fazit Hardware}
Grundsätzlich lässt sich sagen, dass sich die Variation des RAMs vor allem auf die Stabilität des Benchmarks auswirkt. Mehr RAM bringt eine bessere Stabilität. 

Genauso verbessert eine höhere Zahl an genutzten Kernen die Performance. Im Columnstore kann die begünstigte Parallelisierung erst bei mehreren Kernen effizient genutzt werden. Im Rowstore wird die Performance durch wenige Kerne aufgrund der fehlenden Parallelität umso mehr eingeschränkt, da Lesezugriffe auf die Ausführung der Cpu warten müssen und umgekehrt. 